...
abstract class pQL_Driver_Test_Abstract extends PHPUnit_Framework_TestCase {
	...
	function testQueryFilterFunction() {
		$this->exec("DROP TABLE IF EXISTS pql_test_b");
		// схема базы
		$this->exec("CREATE TABLE pql_test(id ".$this->getPKExpr().", val VARCHAR(255))");
		$this->exec("CREATE TABLE pql_test_b(id ".$this->getPKExpr().", val VARCHAR(255), test_id INT)");
		...
		$this->pql()->testB->val->filter('test', function(pQL_Query $query) {
			$query->notLike('%first');
		});
		
		$this->assertEquals(array('b_first', 'b_last'), $this->pql()->testB->val->value()->db()->test->toArray());
		$this->assertEquals(array(5), $this->pql()->test->val->value()->db()->testB->test()->toArray());
	...
	function testForeignKeyNameSave() {
		$this->exec("DROP TABLE IF EXISTS pql_test_b");
		// схема базы
		$this->exec("CREATE TABLE pql_test(id ".$this->getPKExpr().", val VARCHAR(255))");
		$this->exec("CREATE TABLE pql_test_b(id ".$this->getPKExpr().", val VARCHAR(255), test_id INT)");
		...
		$this->assertEquals(2, $this->queryValue("SELECT test_id FROM pql_test_b WHERE id = $b->id LIMIT 1"));
		$this->exec("DROP TABLE IF EXISTS pql_test_b");
	}
	...
	function testObjectPropertyExists() {
		$this->exec("CREATE TABLE pql_test(val VARCHAR(255))");
		$this->assertTrue(isset($this->pql()->test['val']));
		$this->assertFalse(isset($this->pql()->test['wtf']));
		$object = $this->pql()->test();
		$this->assertTrue(isset($object['val']));
		$this->assertFalse(isset($object['wtf']));
		$this->exec("DROP TABLE pql_test");
		
		$this->exec("CREATE TABLE pql_test(fk_id INT)");
		$this->pql->clearCache();
		$object = $this->pql()->test();
		$this->assertTrue(isset($object['fk_id']));
		$this->assertTrue(isset($object['fk']));
	}
